...
| 1.4 | Screens: `home`, `module`, `lesson`, `projects`, `stats`, `help`, `resources`, `workspace`, `settings` | `src/ui/screens/*.jsx` | L | Snapshot parity vs old views' content (same data, new chrome); replay goldens for nav flows | ◐ home, module, stats, help, resources, workspace ported (help shares `src/core/help.js` with the classic view); lesson, projects and settings remain |
