...
- **Motion**: strictly non-essential — e.g. progress meter fill on pass, toast slide — implemented with a single rAF-like interval and fully disabled at Tier C/D and `NO_ANIMATION=1`. Never blocks input. *(Shipped 2026-09-21, scoped to the check flow: `src/ui/animation.js` + `BusyLine`/`CelebrateLine` — the gate is `animationAllowed(caps, env)` (NO_ANIMATION, CI, tiers C/D, non-TTY), the clock is one shared interval in `createAnimator`, and disabled motion renders the static frame the animation would settle on. Meter fill and toast slide remain unshipped — the pass flourish covers the §7.3 intent.)*
...
- **Keystroke-to-paint < 16 ms** (p95) on a mid-range laptop (2020 4-core) for: typing in a 500-line buffer, palette typing, list navigation, browser-tab scrolling. Measured by hooking Ink's stdout write path (not wall-clock around macrotasks — spike A5 showed a naive `setTimeout(0)`-based measure over-counts to ~23 ms p50 on an idle 20-row tree). Asserted in check runs with generous CI headroom (warn > 33 ms, fail > 100 ms). Because idle re-renders can already approach the budget, the editor virtualizes its viewport from day one (§8) and caret moves must not re-render siblings.
  - **⚠ Measured deviation (2026-09-18):** on this machine the 16 ms p95 line **sits at ink 6's own floor**, so it is not a target this UI can hit by writing better screens — it is a property of the renderer. The gate proves it in the same run: a ONE-LINE component (`<Text>`, no editor, no highlighting, no vim) measures **p50 8–10 ms / p95 16–23 ms** at the shipped `maxFps` (the spread is machine load; it straddles 16 ms rather than sitting clearly above or below), i.e. ink's `render()` alone is already at §12's line before any of our code runs. Two costs dominate, both inside ink/its dependency and outside our control: (a) **every paint re-tokenizes the whole frame.** `@alcalzone/ansi-tokenize` walks all frame text with `Intl.Segmenter` and runs an array copy + `filter` per styled character (measured ~0.5 ms per 24-row colored frame row, ~11.5 ms for the editor's frame), and ink creates a fresh `Output` — and therefore a fresh `OutputCaches` — on every render, so the tokenize cache never survives a frame; (b) React's reconciler plus ink's own renderer/`yoga` layout is another ~5 ms. What IS in our control is measured and spent: `maxFps: 240` (ink's 30 default is a ~34 ms trailing edge — p50 35 → 11.5 ms on the floor probe), memoized+reused row elements, one gutter node instead of 24, merged identical adjacent pieces, and the highlighter emitting colours only for keyword/string/number/comment. Result on this machine: **p50 ≈ 36–42 ms / p95 ≈ 80–120 ms** for a 500-line buffer on a quiet run, worse under load (p50 ≈ 58 ms / p95 ≈ 140 ms) — i.e. **~4–7x the same-run one-line floor**, which is the ratio to watch on a loaded machine. The gate therefore reports the floor of the same run next to the editor number, notes above 16 ms, warns at the spec's 33/100 ms lines and fails only on a real regression (≥ 250 ms).
  - **The other three §12 targets are measured too**, on the same floor: **palette typing** (real registry items from `buildPaletteItems` + `SCREEN_TARGETS`, typing a real 9-char word then restarting so the query never ends at "no matches") and **list navigation** (the dashboard's row list, selection moving one row per keystroke) both land at p50 ≈ 30–38 ms / p95 ≈ 60–92 ms, i.e. **~3.6–5.6x the one-line floor** — the same place the editor sits, and far above 16 ms. **Browser-tab scrolling** was added when Phase 3 built the browser (task 3.7): the probe drives the real `BrowserScreen` with a built page and a moving offset, and lands at **p50 ~62 ms / p95 ~159 ms, ~6.3x the floor** — the same wall as the other three. All four §12 targets now exist and are gated.
...
- Frame cost: no full-tree re-render on caret moves — caret blink/move touches the editor component only; Ink's static output used for never-changing chrome where it helps.
  - **⚠ Measured (2026-09-18):** a caret-only move costs ~**0.6x a typing keystroke** (p50 28.6 vs 46.1 ms on the editor replay) rather than being free. Ink re-renders and re-tokenizes the whole tree on ANY update; its `log-update` cursor-only fast path saves the *write*, not the frame build. The component-side half of the contract does hold — a caret move paints byte-identical text (`tests/unit/codeEditor.test.js`: the caret lives in the terminal cursor, never in the text layer) — so the remaining lever is a route-level fast path that updates `sessionRef` and repositions the terminal cursor with no state update at all. Measured payoff ≈ the whole 29 ms per move (the editor is the only thing that reflects the caret; the footer renders hints + mode badge + notice, none of which depend on it). Not taken yet: it must be restricted to moves that change neither the selection nor the scroll view, and it needs `relativeNumbers` to stay off (relative gutter numbers DO depend on the caret row).
...
- **Phase 3 — Browser & tools** ✅ done (tasks 3.1–3.7): render/elements/styles/console tabs ported as-is **plus** live re-render (debounced ~300 ms, error overlay), click-to-inspect (render→elements/styles sync, jump-to-source), network panel (5th tab); console on warmed context (`runConsoleSession` — the learner's code executes once per edit, expressions evaluate on top). The classic `views/browser.js` delete is deferred to the Phase 4 flip for the same reason as Phase 2's: `src/app.js` (the default entry until Phase 4) still imports it, so it is referenced, and check §10 will catch it the moment it is not.
...
| 0.5 | Theme engine: token types, tier mapping (hex→256→16), midnight+paper ports, icons (nerd/unicode/ascii sets) | `src/ui/theme/{types,tiers,icons}.js`, `src/ui/theme/themes/*.js`, `src/ui/theme/context.jsx` | M | Both themes render identical hues to today at 256 tier; tier tests | ✅ done (engine + wiring) — `themes.js` ships the full semantic token set for midnight+paper, `index.js` down-maps hex→`ansi256(n)` for tiers B/C and strips every token at tier D, and `theme/context.jsx` (`ThemeProvider`/`useTheme`, exported through the test harness and mounted by `AppRoot` above the router) hands the resolved tokens to the tree. Every screen and chrome component now asks for a ROLE (`theme.accent`, `theme.muted`, …) instead of an Ink color name, and the routes that build rows pass the theme into the pure `*Lines` helpers — so `FULLSTACK_THEME=paper` repaints the screens (not just `AppRoot`'s status line) and tiers C/D render colorless. Both are asserted in check §9 and `tests/unit/themeContext.test.js`. The icon sets (nerd/unicode/ascii) are the one remaining piece and ride with the Phase 1 screens |
...
| 1.8 | Keymap docs generator + check-tool screen×tier snapshot smoke | `tools/gen-keymap-docs.js`, `tools/check.js` | S | Keymap reference auto-generated matches registry; all screens render on tiers A–D | ✅ done — `tools/gen-keymap-docs.js` renders `docs/keymap.md` from the registry and check §7 fails the run on drift (`npm run keymap:docs`); the screen × tier smoke now runs as check §9, rendering all 11 routes at tiers A–D through an injected `caps` profile (a render smoke rather than a golden snapshot, so it cannot go stale) and asserting that the colorless tiers emit no color SGR at all |
...
| 2.7 | Completions v2: port `complete.js` trigger engine; superset data (HTML tag→attr sets + entities, CSS props/values/at-rules/pseudos, JS DOM + builtins, JSX tags); snippets with tab stops via `applyEdit`; signature help after `(`; popup with mouse + doc line | `src/editor/completions.js`, `src/editor/data/{html,css,js,jsx}.js`, `src/ui/components/CompletionPopup.jsx`, tests | L | Trigger-table tests (Ctrl+Space, ≥2-char auto, `<`/`:`/`@` starters — classic parity); snippet caret placement tests; popup snapshot | ✅ done — the engine (`completionsAt`/`createPopup`/`popupKey`/`acceptItem`/`signatureAt`) is wired into the challenge route through `CompletionPopup.jsx` (task 2.7 finished): a typed character or Ctrl+Space opens the list, `↑↓` select, `Tab`/`Enter` accept as ONE undo step, `Esc` dismisses, snippet tab stops walk with `Tab`, and the signature line renders for a known call. The route intercepts the global `Esc`/`↑`/`↓` while the list is open (§10.3 row 1) and Ctrl+Space is now reachable from the real parser — it arrives as NUL (`\x00`) and was silently dropped before. Acceptance: `tests/unit/completionWiring.test.js` (component render + prefix-opens/Enter-accepts/undo + popup-outranks-global-Esc) plus the existing engine suite |
| 2.8 | `CodeEditor` component: virtualized viewport (visible ±20 lines), memoized per-line highlight (port tokenizer from `widgets.js` → `src/editor/highlight.js`, LRU keyed by line+lang+theme), gutter + line numbers (+relative option), tab strip (Ctrl+Q/W), caret via `useCursor`/explicit `moveTo`, mouse click-caret/drag-select/wheel/tab-click | `src/ui/components/CodeEditor.jsx`, `src/editor/highlight.js`, tests | L | **p95 keystroke-to-paint < 16 ms on a 500-line buffer** (§12 stdout-write-hook method in the replay harness); snapshots at 4 sizes × tiers A–D; wide-char (CJK/emoji) columns verified via wcwidth helper | ✅ done (component) — `CodeEditor` + `highlight.js` (LRU per line) + `viewport.js` render the virtualized window, gutter, tab strip, themed highlight, `useCursor` caret and mouse intents; covered by `tests/unit/codeEditor.test.js`, `highlight.test.js`, `viewport.test.js`. The p95 budget is now MEASURED by check §9 (task 2.11) rather than assumed: 500-line buffer, 60 keystrokes, hooking ink's stdout write path, next to a ONE-LINE control probe that measures ink's own floor in the same run. On this machine that floor is **p50 8–10 ms / p95 16–23 ms**, i.e. §12's 16 ms p95 sits at the edge of what ink@6's renderer can do here at all (the probe straddles the line, 16–23 ms); the editor measures p50 ≈ 36–42 ms / p95 ≈ 80–120 ms on a quiet run (~4–7x that floor; both numbers rise together under load) — see the measured-deviation note in §12 for the per-component breakdown — per-frame re-tokenization by `@alcalzone/ansi-tokenize` is ~11.5 ms of it and is not ours to remove). What is ours has been spent: `maxFps: 120 → 240` (ink's 30 default is a ~34 ms trailing-edge cap, larger than the whole §12 budget; on the floor probe p50 goes 35 → 11.5 → 7.8 ms at 30/120/240), `CodeEditor` memoizes the row subtree and REUSES unchanged row elements, draws the gutter as ONE node instead of 24, merges adjacent identical pieces, and the highlighter colours only keyword/string/number/comment |
| 2.9 | Challenge screen port: `ResizableSplit` brief/checks + editor, pane defaults (`w ≥ 104` rule), full command wiring (check/reset/hint/solution/copy/preview/browser/logs/external-editor with terminal release), **DiffView upgraded to line-level LCS** (replaces positional `diffCode`), per-file save/reset multi-file model, console-log overlay (Ctrl+T) | `src/ui/screens/challenge.jsx`, `src/ui/components/DiffView.jsx`, tests + replay goldens | L | ✅ done — engine + route + replay goldens: LCS diff module (`src/editor/diff.js`, `tests/unit/diff.test.js`), modeless typing layer (`src/editor/typekeys.js`, classic key-path parity), full session ownership in `ChallengeRoute` (draft→starter seed, per-target reseed, gated mouse claim, starter-equal buffers are not drafts, undoable reset, autosave through the debounced write) and the `onRawKey` fallback channel in `useKeymap`. Route-level tests cover navigation, pane memory, typing→autosave→undo, and `tests/unit/editorReplay.test.js` now pins the three replay goldens the spec names — solve-from-scratch, vim insert/undo/redo and the multi-file tab flow — driven through the REAL route (router → vim/typekeys → session → debounced autosave) with intent assertions beside each golden, so a refreshed snapshot cannot encode a wrong result unnoticed. Writing them found three shipped bugs, all fixed: `sessionStep` returned a file NAME that the route stored as the session (`Ctrl+W`/`Ctrl+Q` corrupted it), vim insert/replace mode matched `h`/`j`/`k`/`l` as motions, and `<Esc>` resolved to `app.back` before the editor saw it, which made insert mode a trap. It also exposed a data-loss race: the 400 ms autosave debounce was cleared on unmount, so leaving the challenge right after a keystroke discarded the edit — the route now flushes a pending write on unmount. The Ctrl+T console-log overlay is implemented in `src/ui/screens/challenge.jsx`; only Ctrl+P/Ctrl+E round-trip goldens remain unwritten |
| 2.10 | Footer & mode UX: registry-derived hints for challenge/editor commands, vim mode block (`-- INSERT --` styling), live `editor.vimMode` toggle in settings, one-time footer nudge when typing in normal mode > 2 s | `src/ui/components/Footer.jsx`, `src/ui/screens/challenge.jsx`, tests | M | Hints match registry (lint-enforced); nudge fires once per session; toggle switches modes without restart | ✅ done — `src/ui/hints.js` derives the challenge footer from the registry (`footerHints`/`footerLine`), `keyLabel` renders canonical labels and preserves the case of bare keys (`y` vs `Y` are different vim commands), `modeBadge` drives the `-- INSERT --`/`-- VISUAL --` block, and the route raises the §9 nudge once per mount after 2 s of normal-mode typing. check §7 now feeds each `CHALLENGE_HINT_IDS` binding back through the real `resolveKey` on the challenge screen, so a stranded hint fails the gate. `editor.vimMode` flips live from Settings (no remount) |
| 2.11 | Check-tool integration: perf replay job (N keystrokes → p50/p95 vs §12 thresholds), keymap lint extended to vim bindings (B.8 conflict register), challenge screen × tier snapshot smoke | `tools/check.js` | S | Check green; a synthetic budget breach triggers warn/fail paths; every B.5 binding resolves or is reported | ✅ done — check §7 drives every `VIM_BINDINGS` entry through the real `reduceKey` (a bogus binding fails the run), the docs tables are drift-checked, and the footer hints are resolved back through the registry. check §9 is new: a keystroke-to-paint replay (60 keystrokes into a 500-line `CodeEditor`, hooking ink's stdout write path) reporting p50/p95 **next to a one-line control probe that measures ink's own floor in the same run**, plus the other §12 targets (palette typing, list navigation) and a caret-only replay on the same editor frame, plus a screen × tier render smoke — all 11 routes at tiers A–D render without throwing. Both warnings it first produced are now FIXED rather than recorded: the paint path runs at `maxFps: 120` with memoized/reused editor rows, and the tier C/D color leak is gone because chrome and screens read theme tokens from the new context — check §9 now fails the run if either regresses (`C:0 D:0` colored frames, plus a `FULLSTACK_THEME=paper` assertion that the light theme actually reaches a screen). The only remaining WARN is the honest p95 number: keystroke-to-paint still exceeds §12's 16 ms — which §12 now records as a measured deviation, since the gate's one-line control probe shows ink 6's own floor is already at it here (p95 ~16–23 ms depending on load, so the probe straddles the line rather than clearing it). The gate prints that floor beside the editor number and fails only past a 250 ms regression ceiling. The smoke also caught a real crash: the theme's 256-color down-map passed a NUMBER to ink (`color.startsWith is not a function`); it now emits ink's `ansi256(n)` string form. The harness is no longer a `--spike`-only artifact — `npm run build` always emits `dist/harness.js`, so these gates cannot silently skip |
| 2.12 | Delete pass + docs: remove `src/views/editor.js` model, old challenge rendering, and `widgets.js` codeBlock/tokenizer once unreferenced (classic challenge screen still default until Phase 4 flip); README vim cheatsheet + regenerated keymap table | `src/views/*`, `README.md`, `CONTRIBUTING.md` | S | No dead exports; `npm run build && npm test && npm run check` green; docs match implemented keys | ◐ partial, and **split by design** — the sweep half is DONE, the classic-delete half is deferred to the Phase 4 flip because the word "once unreferenced" is a fact, not a plan: `src/app.js` is still the default entry (`FULLSTACK_UI=next` is the opt-in) and it imports `src/views/editor.js` (the modeless key path, `emptyEditors`/`editorText`/`move`/…) while every classic view imports `tui/widgets.js`, so deleting them today would break the shipping UI. What the sweep covers now is the half that CAN be unreferenced: **5 dead exports removed** (`JSX_KEYWORDS`, `cursorsAt`, `isRegisterPrefix`, `VISUAL_MODES`, `confirmSubstituteState`), and check §10 makes "no dead exports" a standing gate instead of a one-off audit — it scans `src/editor/**` + `src/ui/**` for exports referenced nowhere in the project (src, tests, tools, docs, root configs — a helper used only by a test is alive) and for modules nothing imports, fails on either, and carries an explicit allowlist for **documented seams** (`probeGraphicsTTY`, which `docs/multimedia.md` says exists for the M1 screenshot feature that is not wired yet). The gate was verified to FAIL by planting a dead export, not just to pass. Docs side: `npm run keymap:docs` regenerates `README`-linked `docs/keymap.md` + `docs/vim.md` with no diff (the §7 drift gate already keeps them honest), and `check §7` still resolves all 33 vim bindings plus the challenge footer hints through the real registry. **Deferred to the Phase 4 flip:** the `src/views/editor.js` model, the old challenge rendering path and `widgets.js` codeBlock/tokenizer — with the same sweep as the acceptance test, since §10 fails on any module that outlives its last referent |
...
> **Progress note (2026-09-18):** **Tasks 2.1–2.11 are landed; Phase 2's remaining work is 2.12 (the delete pass) plus the editor replay goldens and the two quality items the new gates surfaced.** The engine (2.1–2.6) was already in place; this round closed the UI half: **2.7** completions are wired end to end (a `CompletionPopup` component, trigger-on-type + Ctrl+Space, `↑↓`/`Tab`/`Enter`/`Esc` priority over the global table, snippet tab stops, signature line — and Ctrl+Space is finally reachable, since it arrives as NUL and the byte parser used to drop it), **2.8**'s `CodeEditor` and **2.9**'s route were already exercised, **2.10** finished the footer/mode UX (`keyLabel` case-preservation + registry-derived hints linted in check §7 + the mode badge/nudge), and **2.11** added check §9.
>
> §9 is deliberately two jobs in one section: a **keystroke-to-paint replay** (60 keystrokes into a 500-line buffer, measuring to the next ink stdout write — §12's method, not wall-clock around a macrotask) and a **screen × tier smoke** (all 11 routes rendered at tiers A–D with an injected capability profile; `AppRoot` gained a `caps` override because the real probe reads `process.stdout.isTTY`, which is always false under a piped check). It earned its keep immediately by catching a shipped crash: the theme's hex→256 down-map set color tokens to NUMBERS and ink's colorizer calls `color.startsWith(...)` on them, so **tier B/C rendering threw** — never exercised before because nothing rendered below tier A. It now emits ink's `ansi256(n)` form.
>
> Two findings were reported as WARNs rather than hidden, because neither was a one-line fix: (a) **ink@6's `render()` throttles at `maxFps: 30`**, a ~34 ms floor that put p95 keystroke-to-paint at ~70–160 ms — fixed by `maxFps: 120` and the editor's row reuse (see the update-2 note above); (b) **tiers C/D still emitted color** on all 11 screens because chrome used hardcoded Ink color names (`cyan`, `green`, …) alongside theme tokens — fixed by the theme context, and now a hard failure instead of a warning. `npm run build` always builds `dist/harness.js` (it is test infrastructure, not a spike), so §9 and the unit suites cannot silently skip in CI.
>
> One pre-existing hazard was recorded for 2.12 and is now closed: two printable bytes arriving in the SAME stdin chunk used to be handled synchronously against the same render's `session`, so the second could overwrite the first (`setSession(next)` passed a value, not an updater). Every key path in `ChallengeRoute` now reads and writes the session through a ref (`sessionRef`), so a burst composes in order — pinned by the two-key-burst test in `tests/unit/routes.test.js`.
>
> **Progress note (2026-09-18, update 2):** **Both quality items the §9 gate surfaced are fixed, and the burst-typing hazard is closed.** (1) *Theme tokens*: `src/ui/theme/context.jsx` (`ThemeProvider`/`useTheme`) replaces every hardcoded Ink color NAME across 8 components and 12 screens with theme roles; the routes that build rows (`LessonRoute`, `ProjectsRoute`, and `ChallengeRoute`'s LCS diff) pass the resolved theme into the pure `*Lines` helpers as a parameter, so those helpers stay pure and directly testable while still matching what the screen paints. `FULLSTACK_THEME=paper` now repaints the screens, and the tier smoke's colored frames went from `A:11 B:11 C:11 D:11` to `A:11 B:11 C:0 D:0` — the C/D leak is asserted as a FAILURE now, not a warning, and a paper-theme frame is asserted to carry the paper accent while the midnight frame must not. `tests/unit/themeContext.test.js` covers provider propagation, the no-provider default and tier-D degradation. (2) *Perf*: `main.jsx` renders with `maxFps: 120` (ink's 30 default caps paint at a ~34 ms trailing edge — larger than the entire §12 budget) and `CodeEditor` memoizes the row subtree and reuses unchanged row elements, so a keystroke re-renders one row instead of ~24; check §9's replay moved from p50 ≈ 56 ms to p50 ≈ 30 ms / p95 ≈ 76 ms on this machine (superseded by update 4, which re-measured with `maxFps: 240` and a control probe for ink's own floor). (3) *Burst typing*: the session lives behind `sessionRef`, so two printable bytes in one stdin chunk compose instead of the second overwriting the first.
>
> **Progress note (2026-09-18, update 3):** **2.9's replay goldens land, and they found three shipped bugs plus one data-loss race.** `tests/unit/editorReplay.test.js` mounts the real route tree (router → `ChallengeRoute` → vim/typekeys → session → debounced autosave) with fake services and replays three scripted sequences, writing `tests/snapshots/editor-replay-*.snap` (buffers + the painted viewport) beside intent assertions — the golden is the reviewable form of the behaviour, the assertion is what stops a refresh from encoding a wrong result. (1) `sessionStep` (`editor.tabNext`/`tabPrev`) returns a file NAME, but the route assigned its result as the session, so `Ctrl+W`/`Ctrl+Q` replaced the session object with a string — fixed by looking the real state up from the returned name. (2) vim's insert/replace arm fell through to `ARROW_TO_MOTION`, so typing `h`, `j`, `k` or `l` moved the cursor instead of inserting the letter — fixed, with the acceptance table extended. (3) `<Esc>` resolves to the global `app.back` BEFORE the screen's handler, so with vim on, insert mode could not be left without popping the screen; the route now forwards Esc to the vim machine while the editor is mid-mode (normal-mode Esc still pops, asserted at the end of the vim case). (4) The replay's fast-exit path exposed a real race: the 400 ms autosave debounce was cleared by the effect cleanup on unmount, so Esc-ing off the challenge within the window discarded the edit — the write is now kept in a `pendingSave` ref and flushed on unmount. The single-file/`starter` draft shape and the multi-file `files` map are both covered, and the goldens document the letters that are registry bindings (`g`/`G`/`y`) and therefore never reach the typing path.
>
> **Progress note (2026-09-18, update 4 — painting cost, and what §12 really costs):** the open perf item is now pinned down rather than chased. check §9 grew a **control probe**: the same keystroke-to-paint measurement on a one-line `<Text>` component, rendered in the same process with the same `maxFps`. It reports p50 8–10 ms / p95 16–23 ms, which settles the question — **§12's `< 16 ms` p95 sits at `ink@6`'s own floor on this machine**, so no screen-level optimisation could reach it. A CPU profile of the editor replay attributes the rest: `@alcalzone/ansi-tokenize` ~11.5 ms per 24-row frame (ink re-tokenizes every text node on every paint because `OutputCaches` is created inside the per-render `Output`), React's reconciler ~4.5 ms, ink's own renderer ~5 ms, `yoga` ~1.5 ms. Three fixes did land, all measurable: the gutter is ONE multi-line text node instead of 24 (24 fewer nodes per frame), `rowPieces` output is merged across adjacent identical styles (a whole-line selection or a block comment was painting as many nodes as it had cuts), and `maxFps` went 120 → 240 — the floor probe shows the throttle is the dominant term for LIGHT frames (p50 35 ms at ink's default 30, 11.5 ms at 120, 7.8 ms at 240) while a heavy editor frame is CPU-bound and unaffected, and since a paint costs ≥ 5 ms of CPU the write rate an interactive session can reach stays far below the cap. Editor replay p50 went ~44 → ~36–42 ms; §12 now carries a **measured-deviation** paragraph with the numbers so the remaining WARN is a documented property of the renderer, not an open promise. The gate prints the floor next to the editor number so the absolute values stay interpretable across machines.
>
> **Progress note (2026-09-18, update 5 — the rest of §12, and the 2.12 sweep half):** the perf gate now covers **all the §12 targets that exist**, not just the editor: **palette typing** (the real `buildPaletteItems` registry list, query typed a real word at a time so the filter never settles into "no matches") and **list navigation** (the dashboard's rows, selection moving one per keystroke) each replay 40 keystrokes against the same-run one-line floor. They land at p50 ~30-38 ms / p95 ~60-92 ms — **~3.6-5.6x the floor**, i.e. the same relationship the editor has, which is the point: §12's four targets share one wall, and it is ink's, not the screen's. Browser-tab scrolling gets a probe when Phase 3 builds the browser. **Caret moves are measured rather than assumed**: §12's "caret moves must not re-render siblings" is currently only half true — a caret-only move is **~0.6x a typing keystroke** (p50 28.6 vs 46.1 ms), because ink re-renders and re-tokenizes the whole frame on any update and its cursor-only `log-update` path saves the write, not the build. The component half of the contract is now asserted (`tests/unit/codeEditor.test.js`: a caret move paints byte-identical text, since the caret lives in the terminal cursor and never in the text layer), and the remaining lever — a route-level fast path that updates `sessionRef` and moves the terminal cursor with no state update, worth the whole ~29 ms per move — is written down with its guard conditions (no selection change, no scroll, `relativeNumbers` off) in §12 rather than half-built. Finally, the **2.12 sweep half is done**: five dead exports deleted (`JSX_KEYWORDS`, `cursorsAt`, `isRegisterPrefix`, `VISUAL_MODES`, `confirmSubstituteState`) and check **§10** makes "no dead exports" standing — it fails on any export in `src/editor/**` or `src/ui/**` referenced nowhere across src/tests/tools/docs/root configs, and on any module nothing imports, with a named allowlist for documented seams (`probeGraphicsTTY`, the M1 screenshot probe). Verified by planting a dead export and watching it fail. The classic-model delete stays deferred to the Phase 4 flip: `src/app.js` is still the default entry and imports `src/views/editor.js`, so those files are referenced — §10 is what will catch them the moment they are not. `npm run keymap:docs` regenerates the README-linked keymap + vim cheatsheets with no diff, and §7 still resolves all 33 vim bindings and every challenge footer hint through the real registry.
>
> **Progress note (2026-09-18, update 6 — Phase 3 lands):** **the browser & dev tools are ported** (tasks 3.1–3.7). `src/ui/screens/browserModel.js` is the pure row model (layout stamp → elementTree row → source map, clickable ⚠ issue rows, the five panes), `browser.jsx` is the presentational screen, and `routesBrowser.jsx` owns the session: Ctrl+B from the challenge pushes it, live re-render polls the autosaved draft at the spec's 300 ms and turns a layout throw into the ⚠ overlay, click-to-inspect syncs Render→Elements/Styles, Enter jumps to the element's source (handed to the still-mounted challenge route via `browserJumpHandoff`), and the console runs on `runConsoleSession` so the learner's code executes once per edit and later expressions evaluate on top of its bindings. Two defects surfaced while making the task-3.7 acceptance test green, both fixed in the route: (1) printable keys that resolve to NO command (`+`, `(`, `=`) were dropped before the console saw them — `useKeymap` now gets an `onRawKey` that routes them into the console input, sharing one `touchConsole` predicate with the command path so a bound key (`j`, `1`…`5`) and an unbound key behave the same; (2) the session ref was only refreshed at render, so two keys in one stdin chunk raced the way ChallengeRoute's did — the browser route now uses the same synchronous setter wrapper. The console owns printable keys, so pane switching there is `Tab` (`browser.tabNext`), recorded in the console hint. The §12 gate gained its fourth target — **browser-tab scrolling** drives the real `BrowserScreen` over a built page with a moving offset, and `BrowserScreen`'s rows are memoized on absolute pane indices so a scroll only re-renders the newly-exposed rows; it lands at p50 ~62 ms / p95 ~159 ms (~6.3x the same-run floor), WARN-only like the other three. A `buildTree` helper that nothing used was deleted (check §10 caught it). `tests/unit/routesBrowser.test.js` (route) + `tests/unit/browserModel.test.js` (model) cover it; `npm run check` and all 758 unit assertions are green. The classic `src/views/browser.js` stays referenced by `src/app.js` (default entry until the Phase 4 flip) and is deleted there, same as the Phase 2 model.
