...
| 1.4 | Screens: `home`, `module`, `lesson`, `projects`, `stats`, `help`, `resources`, `workspace`, `settings` | `src/ui/screens/*.jsx` | L | Snapshot parity vs old views' content (same data, new chrome); replay goldens for nav flows |
| 1.5 | Command palette (fuzzy over registry + "Go to" sources, recent list) on `<C-k>` | `src/ui/screens/palette.jsx` | M | Fuzzy perf test (<2 ms/200 items); `<C-p>` no longer opens palette outside challenge |
