...
- **Editor engine (Phase 2, in progress):** `src/editor/` is framework-agnostic and pure, built in the spec's order. So far: `document.js` (line array behind a single `applyEdit(changes)` chokepoint; `{changes, caret}` transactions; insert/delete/replace/setText wrappers; a multi-file session holding one document + history + view state per file, so switching tabs restores caret, scroll and undo) and `history.js` (undo/redo over exact before/after snapshots, with coalescing of typing/pair/indent runs inside a 400 ms window, always-separate paste/completion/replace-all steps, redo cleared on a new edit, 1000-step cap) and `width.js` (zero-dep wcwidth-style cells for CJK/emoji/combining marks, tab stops, click-to-caret and horizontal slices). Nothing is wired to a screen yet — the vim state machine (2.4) and `CodeEditor` (2.8) come next.
- **Not ported (Phase 2+):** the editor UI itself (vim, completions, highlighting), the browser tabs, and the remaining themes/icons.
...
Errors & QoL (`errors-and-qol-spec.md`): the classic scope is complete (Q1–Q10, Q13, Q14 shipped with replays in `tools/check.js` §6/§8), and Q7/Q10/Q13 now also work on the Ink UI (results header with duration + micro-notes, suggest-only format, quit recap — `tests/unit/routes.test.js`); the palette and tour screens have landed, so what remains on that list is Q12's `%` binding, which needs the Phase 2 vim engine.
Next UI (`FULLSTACK_UI=next`): Phase 1 is complete (shell, navigation, palette, tour, resizable panes). What is left is the Phase 2 editor (vim, undo, selection, search, multi-cursor, snippets), then the Phase 3 browser and the Phase 4 cut-over.
