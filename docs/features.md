...
- **Screens ported:** dashboard, module, lesson and challenge routes are real (`src/ui/routes.jsx`: `j/k/g/G`, Enter opens, Esc pops, Ctrl+S runs the real grader, Ctrl+H/Ctrl+G hint + solution, record-backed buffer/reset) plus **Projects** (capstone checklists: `j/k` project, Tab focuses the checklist, Space ticks) and the read-only scroll screens **Help** (`?`), **Resources**, **Workspace** and **Progress** (`j/k` scroll, `g`/`G` ends). Help renders `src/core/help.js`, the same content the classic view lays out. The lesson screen keeps the classic flow — module → lesson → challenge — with `Tab` cycling the practice focus and `m`/`n` for read-state and next-lesson.
...
- **Still to port:** the settings screen, resizable-split persistence (1.2) and the welcome tour (1.6) — named in `UNPORTED_SCREENS`.
