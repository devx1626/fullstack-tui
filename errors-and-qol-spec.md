...
| **Q5. Milestone celebrations** | One-time toasts + a dashboard flourish for: streak 7/30/100, new personal best streak, module completion 25/50/75/100%. Seen-milestones tracked in `settings.milestonesSeen` (id-keyed) so each fires exactly once. Ink parity (Phase 4): the challenge route calls the same `takeMilestones` after a pass and shows `★ 7-day streak!`-style toasts through `host.say`; the id → text step is shared with the classic note (`src/ui/milestones.js`), icon-aware so the ascii set stays 7-bit | 1 | Replay: seed store at streak 6, simulate a pass crossing 7 → assert toast shown once; re-run → assert not shown again. Ink: `tests/unit/milestoneToasts.test.js` drives the real ChallengeRoute (streak 6→7 → toast once; second check and a second session stay silent; seen ids persisted) |
...
| **Q12 (shipped, Ink bell)** | ✅ The visible bell lives at the Ink global layer post-flip (loose-ends-spec P1-12): `dispatchGlobal`'s fallthrough rings a status-line note (`'z' does nothing here — '?' lists the keys`, muted tone, ~1s same-key suppressor) via the host's `say` channel (`src/ui/host.jsx` bell sink); typing surfaces and `?` stay exempt by structure (they claim or resolve first). Replay: `tests/unit/routes.test.js` subtests P1-12 (bell once + re-arm, `?` opens help, editor/console silent). The `?` binding required `parseBinding` to accept bare punctuation — it had parsed as null, leaving the shipped `?` hint dead in the resolver | — | done |
...
| **Phase 0 (+0.7a)** | Registry skeleton + QoL command registration; Q3 endpoints live | Registry smoke replays green | ✅ done — the registry shipped with 46 ids at the time (67 today) + conflict lint; Q1/Q5/Q6/Q8/Q14 replays green; settings store shipped |
| **Phase 1 (+QoL)** | Q1, Q2, Q4 (text), Q5, Q6, Q7, Q8, Q9, Q10 (classic), Q13, Q14; palette consumes registry | Full QoL contract green on classic | ✅ classic scope done — Q1–Q10 and Q13/Q14 all shipped with unit + replay coverage (check §6/§8): recents MRU, `nav.nextUp`, `g`/`G` endpoints, `→ line N` + caret jump, milestones, banner, autosave, checkpoints + `history.restore`, goal bar, Q7 run timing + micro-notes, Q10 suggest-only format note, Q13 on-quit recap. The palette now renders the *same* item list the cursor selects from. The Ink port of this phase is complete (palette, tour, settings and the resizable split all landed), so nothing on this row is outstanding |
| **Phase 2 (+QoL)** | Q4 (caret jump), Q11 soft-wrap (Appendix E 2.8 addendum), Q12 | Phase 2 gate + soft-wrap unit/replay | ✅ Q4 caret jump (`Ctrl+J`), Q11 soft-wrap and Q12 complete: the classic halves shipped with unit + replay coverage (`src/core/softwrap.js`, `src/core/brackets.js`, `tests/unit/softwrap.test.js`, `tests/unit/brackets.test.js`, `tests/unit/editor.test.js`, Settings → Wrap / Tab size), the `%` binding went live with the Phase 2 vim engine, and the visible bell now answers ignored keys on the Ink UI too (Q12 Ink bell row above, loose-ends-spec P1-12). `editor.bracketMatch` exists, is tested, and is reachable from the palette |
| **Phase 4** | Recap & toasts reproduced on the Ink UI at cut-over; flourishes (decoration tier) for Q5 | Cut-over checklist includes QoL parity | ◑ nearly done — Q13 recap is on Ink (`main.jsx` quit path), Q5 milestone toasts are on Ink (`ChallengeRoute` pass branch → `host.say`, shared `milestoneToast` text with the classic note, `tests/unit/milestoneToasts.test.js`); the §7.3 flourish set (busy spinner + pass sparkles) landed with the motion gate. Remaining: M2 curly underlines (docs/multimedia.md) |
